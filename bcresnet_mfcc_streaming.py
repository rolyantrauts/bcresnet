...
# --- Full Network (Dynamic Freq Support) ---

class StreamingBCResNet(nn.Module):
    def __init__(self, n_classes=12, tau=1.0, input_freq=40):
        super().__init__()
        ...
        # --- Dynamic Classifier Calculation ---
        # Calculate final feature map height based on input frequency
        # Stem: Stride 2, Pad 2, Kernel 5
        h = (input_freq + 4 - 5) // 2 + 1
        # Stage 0: Stride 1 (No change)
        # Stage 1: Stride 2, Pad 1, Kernel 3
        h = (h + 2 - 3) // 2 + 1
        # Stage 2: Stride 2, Pad 1, Kernel 3
        h = (h + 2 - 3) // 2 + 1
        # Stage 3: Stride 1 (No change)
        
        if h < 1: h = 1 # Safety
        
        # print(f"DEBUG: Input Freq {input_freq} -> Final Feature Map Height {h}")

        last_c = self.stage_channels[4]
        final_c = self.stage_channels[5]
        self.classifier = nn.Sequential(
            # Adapts kernel size to whatever h is (e.g. 3 for 20-bin input)
            nn.Conv2d(last_c, last_c, kernel_size=(h, 1), padding=(0, 0), groups=last_c, bias=False),
...
def export_onnx(model, path="bcresnet_mfcc_streaming.onnx"):
    model.eval()
    
    # Use the model's configured input frequency
    freq = model.input_freq
    dummy_input = torch.randn(1, 1, freq, 1)
    ...
    print(f"Exporting model (Input: 1x{freq}x1) with {len(dummy_states)} state buffers...")
