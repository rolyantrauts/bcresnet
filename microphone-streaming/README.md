...
⚡ ExamplesHigh-Performance Setup (Raspberry Pi 4 / PC)Scans audio every 50ms for ultra-fast response.Bashpython stream_test.py --model bcresnet_float32.onnx --poll_rate 0.05
Low-Power Setup (Raspberry Pi Zero 2)Uses the Int8 model, scans every 200ms, and uses a longer averaging window to ensure stability on the weaker CPU.Bashpython stream_test.py --model bcresnet_int8.onnx --poll_rate 0.2 --average_window 5
Custom Training SettingsIf you trained a model with specific settings (e.g., 1.0 second duration), you must match them here:Bashpython stream_test.py --duration 1.0 --sample_rate 16000
❓ Troubleshooting"Error opening InputStream"Run python -m sounddevice to see your device list.Find the ID of your USB mic and run: python stream_test.py --device <ID>"IndexError: list index out of range"Your --wakeword_idx might be wrong.Check your training folders alphabetical order.Example: ['background', 'computer', 'speech'] -> 'computer' is index 1.Run: python stream_test.py --wakeword_idx 1"Model input shape mismatch"Ensure --duration and --sample_rate exactly match the arguments you used in main.py during training.