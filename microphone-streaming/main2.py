...
class Trainer:
    ...
    def export_onnx(self):
        print("\n--- Exporting to ONNX (NUCLEAR + STATIC SHAPE) ---")
        ...
        try:
            # --- FIX: REMOVED 'dynamic_axes' ---
            # By removing dynamic_axes, we force the model to have a fixed input size.
            # This allows AdaptiveAvgPool to calculate a fixed kernel size, solving the error.
            torch.onnx.export(
                self.model, 
                dummy_spectrogram, 
                f32_path, 
    ...
    def __call__(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for inputs, labels in loop:
                ...
                # Preprocess (Augment) on GPU/CPU before passing to model
